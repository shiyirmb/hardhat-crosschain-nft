...
task("burn-and-cross", "燃烧WNFT并进行跨链")
    ...
    .setAction(async(taskArgs, hre) => {
        ...
        if (!receiver) {
            // 获取目标链 sepolia NFTPoolLockAndRelease 合约
        ...
        const burnAndMint = await ethers.getContract("NFTPoolBurnAndMint", account1)
        
        // 转移 LINK 通证到池子中 需要先在https://faucets.chain.link/ 获取 Polygon Amoy Link 通证
        ...
        const oldBalance = await linkToken.balanceOf(burnAndMint.target)
        console.log(`linkToken合约-当前LINK数量:${oldBalance}, 开始转移Link通证...`)
        // 没有LINK通证 需要转移通证到池子里
        const linkTokenTx = await linkToken.transfer(burnAndMint.target, ethers.parseEther("10"))
        await linkTokenTx.wait(CONFIRMATIONS)
        const newBalance = await linkToken.balanceOf(burnAndMint.target)
        console.log("转移通证成功, 当前LINK数量:", newBalance)

        // 授权
        console.log("授权中...")
        const wnft = await ethers.getContract("WrappedMyToken", account1)
        await wnft.approve(burnAndMint.target, tokenid)
        console.log("授权成功")

        // 发送CCIP跨链消息
        console.log("消息发送中...")
        const burnAndCrossTx = await burnAndMint.burnAndSendNFT(tokenid, account1, chainselector, receiver)
        console.log("消息发送成功,交易hash:", burnAndCrossTx.hash)
    })
